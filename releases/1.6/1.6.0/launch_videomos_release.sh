...
export VIDEOMOS_DBCACHE_GB=$VIDEOMOS_CACHEDB_GB
mkdir ${SHARED_FOLDER}
./create_webgrabplus_volumes.sh
docker login
docker-compose -f docker-compose-release.yaml pull
docker-compose -f docker-compose-release.yaml -p vmos up --no-build --no-recreate -d webgrabplus
docker-compose -f docker-compose-release.yaml -p ${API_PORT} up --no-build --no-recreate -d sonda websonda mongo
