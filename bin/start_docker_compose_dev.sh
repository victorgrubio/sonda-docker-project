...
docker rm -f ${API_PORT}_sonda_1 ${API_PORT}_websonda_1 ${API_PORT}_mongo_1 ${API_PORT}_webgrabplus_1
mkdir $ROOT_FOLDER/tmp && mkdir ${SHARED_FOLDER}
docker-compose -f $ROOT_FOLDER/docker/docker-compose-dev.yaml -p ${API_PORT} build
docker-compose -f $ROOT_FOLDER/docker/docker-compose-dev.yaml -p ${API_PORT} up
